...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch('http://localhost:8090/api/rag/queryRagTagList')
...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
 ...
 function startEventStream(message) {
    ...
    if (ragTag) {
        url = `http://localhost:8090/api/${aiModelValue}/generateStreamRag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `http://localhost:8090/api/${aiModelValue}/generateStream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
